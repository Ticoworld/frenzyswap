    ...
    <section className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-200 dark:border-gray-700" aria-labelledby="privacyCtrlHeading">
      ...
      <div className="grid md:grid-cols-2 gap-4 text-sm">
        <label className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700/60 rounded p-3">
          ...
          <input type="checkbox" checked={hidePnl} onChange={e => { const v = e.target.checked; setHidePnl(v); persist('frenzy_priv_hide_pnl', v); void sync({ hide_pnl: v }).then(ok => { if (ok && didMount) toast.success('Saved'); }); }} className="h-4 w-4 accent-frenzy dark:accent-gray-600" />
        </label>
        <label className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700/60 rounded p-3">
          <span className="text-gray-700 dark:text-gray-300">Hide Volume</span>
          <input type="checkbox" checked={hideVolume} onChange={e => { const v = e.target.checked; setHideVolume(v); persist('frenzy_priv_hide_volume', v); void sync({ hide_volume: v }).then(ok => { if (ok && didMount) toast.success('Saved'); }); }} className="h-4 w-4 accent-frenzy dark:accent-gray-600" />
        </label>
        <label className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700/60 rounded p-3">
          <span className="text-gray-700 dark:text-gray-300">Hide Badges</span>
          <input type="checkbox" checked={hideBadges} onChange={e => { const v = e.target.checked; setHideBadges(v); persist('frenzy_priv_hide_badges', v); void sync({ hide_badges: v }).then(ok => { if (ok && didMount) toast.success('Saved'); }); }} className="h-4 w-4 accent-frenzy dark:accent-gray-600" />
        </label>
        <label className="flex items-center justify-between bg-gray-50 dark:bg-gray-900/40 border border-gray-200 dark:border-gray-700/60 rounded p-3">
          <span className="text-gray-700 dark:text-gray-300">Analytics opt-out</span>
          <input type="checkbox" checked={optOut} onChange={e => { const v = e.target.checked; setOptOut(v); persist('frenzy_analytics_opt_out', v); void sync({ analytics_opt_out: v }).then(ok => { if (ok && didMount) toast.success('Saved'); }); }} className="h-4 w-4 accent-frenzy dark:accent-gray-600" />
        </label>
      </div>
