      ...
      >
        ...
        <div className="ml-3 text-left truncate flex-1">
          <div className="flex items-center space-x-2">
            <span className="font-medium text-gray-900 dark:text-white truncate">{token.symbol}</span>
            <TokenVerificationBadge token={token} size="sm" />
          </div>
          <div className="text-sm text-gray-600 dark:text-gray-400 truncate">{token.name}</div>
        </div>
      </button>
    );
  ...
  return (
    <div className="token-selector-wrapper"> {/* Renamed for clarity, not strictly needed for scrollbar */}
      <button
        ...
        className="w-full bg-gray-100 dark:bg-gray-800 p-3 rounded-lg text-gray-900 dark:text-white flex items-center justify-between hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-gray-900"
      ...
      <Transition show={isOpen} as={Fragment}>
        <Dialog onClose={() => setIsOpen(false)} className="fixed inset-0 z-50">
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Dialog.Panel className="w-full max-w-md bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700">
              <div className="p-6">
                ...
                <input
                  ...
                  className="w-full p-3 mb-4 bg-gray-100 dark:bg-gray-700 rounded-lg text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-gray-400 border border-gray-200 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:bg-white dark:focus:bg-gray-600 focus:border-yellow-500"
                  autoFocus
                ...
                <div className="space-y-2">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <div key={i} className="flex items-center p-3">
                      <div className="w-8 h-8 mr-3 bg-gray-200 dark:bg-gray-700 rounded-full animate-pulse" />
                    ...
                    <div className="flex flex-col items-center justify-center h-full p-6 text-center">
                      {searchFeedback.isSearching ? (
                        ...
                        <>
                          ...
                          <p className="text-gray-700 dark:text-gray-300 font-medium mb-2">No tokens found</p>
                          <p className="text-gray-500 dark:text-gray-500 text-sm leading-relaxed max-w-sm">
                          ...
                          <button
                            ...
                            className="mt-4 text-yellow-600 dark:text-yellow-500 hover:text-yellow-500 dark:hover:text-yellow-400 text-sm underline"
                          >
                            Clear search
                          </button>
                        </>
