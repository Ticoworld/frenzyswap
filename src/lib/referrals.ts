...
export async function createReferral(referrer: string, referee: string) {
  ...
  const { data: bl } = await client.from('blacklisted_wallets').select('wallet_address').in('wallet_address', [referrer, referee]).limit(2)
    ...
    const { data, error } = await client
      .from('referrals')
      // NOTE: schema uses 'referred_wallet' column per migration; keep consistent
      .insert([{ referrer_wallet: referrer, referred_wallet: referee, status: 'pending' }])
      .select()
...
export async function verifyReferralIfAny(referee: string) {
  ...
  const { data } = await client
      .from('referrals')
      .select('*')
      // NOTE: migration uses 'referred_wallet'
      .eq('referred_wallet', referee)
      .limit(1)
    const ref = data?.[0]
